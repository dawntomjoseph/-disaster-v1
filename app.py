...
def register_taluk():
    ...
    taluk_id = cur.lastrowid
    
    # Initialize resources for the new taluk
    cur.execute(
...
def get_requests():
    ...
    cur.execute("""
    SELECT r.id, t.name as taluk, r.resource_type,
           r.quantity, r.status
    FROM resource_requests r
    JOIN taluk t ON r.taluk_id = t.id
    """)
...
def accept_request():
    ...
    cur.execute("SELECT * FROM resource_requests WHERE id=?", (request_id,))
    req = cur.fetchone()

    if not req:
        return jsonify({"error":"Request not found"}),404

    taluk_id = req["taluk_id"]
...
def reject_request():
    ...
    data = request.get_json()
    request_id = data.get("request_id")
    reason = data.get("reason", "Manually rejected by admin")
...
def request_resource():
    ...
    cur.execute("""
    INSERT INTO resource_requests (taluk_id,resource_type,quantity)
    VALUES (?,?,?)
    """,(taluk_id,resource,quantity))
...
def get_all_requests():
    ...
    cur.execute("""
    SELECT r.id,t.name as taluk,r.resource_type,r.quantity,r.status,r.reason
    FROM resource_requests r
    JOIN taluk t ON r.taluk_id=t.id
    """)
...
def approve_request():
    ...
    cur.execute("SELECT * FROM resource_requests WHERE id=?", (req_id,))
    req=cur.fetchone()
    ...
    # Check total available resources from other taluks
    cur.execute(f"""
    SELECT SUM({resource}) as total_available
    FROM resources
    WHERE taluk_id != ?
    """, (taluk_id,))
    ...
    if total_available < qty:
        ...
        cur.execute("""
        UPDATE resource_requests
        SET status='REJECTED', reason='Insufficient resources available from other taluks'
        WHERE id=?
        """,(req_id,))
        conn.commit()
        return jsonify({"status":"rejected", "reason": "Insufficient resources available"})
...
@app.route('/api/get_taluk_requests/<taluk_name>')
def get_taluk_requests(taluk_name):
    conn = get_db()
    cur = conn.cursor()
    ...
    cur.execute("""
    SELECT id, resource_type, quantity, status, reason
